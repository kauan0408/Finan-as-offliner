...
function tipoLabel(tipo) {
  ...
  if (tipo === "nulo") return "Sem efeito no saldo";
...
export default function HistoricoPage() {
  ...
  const resultado = useMemo(() => {
    ...
    let listaBase = [...transacoes].filter(
      (transacao) => transacao?.origemMovimento !== "quem_me_deve"
    );
    ...
    if (!dataInicio && !dataFim && mesReferencia) {
      ...
      (Array.isArray(transacoes) ? transacoes : [])
        .filter((t) => t?.origemMovimento !== "quem_me_deve")
        .forEach((t) => {
        const dt = parseDateValue(t.dataHora);
        ...
        <div className="modal-overlay" onClick={() => setMostrarAjuda(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="muted small" style={{ marginTop: 14, lineHeight: 1.8 }}>
              ...
              <div>• Reembolso aumenta o saldo porque o dinheiro entrou, mas não é contado como receita normal.</div>
              <div>• Sem efeito no saldo mostra registros que não alteram o dinheiro disponível.</div>
          ...
          <div className="modal-card">
            ...
            <div className="field">
              ...
              <select value={tipoEdit} onChange={(e) => setTipoEdit(e.target.value)}>
                ...
                <option value="nulo">Sem efeito no saldo</option>
              </select>
            </div>

            {/* Categoria aparece para despesa e reembolso */}
            {(tipoEdit === "despesa" || tipoEdit === "reembolso") && (
              <div className="field">
                <label>Categoria</label>
                <select
