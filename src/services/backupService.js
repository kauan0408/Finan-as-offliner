...
function itemEstaNoPeriodo(item, filtroRecebido) {
  ...
  if (filtro.modo === "dias") {
    ...
    inicio.setDate(
      inicio.getDate() - filtro.quantidade + 1
    );
...
function filtrarLista(lista, filtro, manter) {
  ...
  return lista.filter((item) => {
    const estaNoPeriodo = itemEstaNoPeriodo(
      item,
      filtro
    );
  ...
  if (chave === "financas_quem_me_deve") {
    ...
    return {
      ...
      pessoas: Array.isArray(valor?.pessoas)
        ? valor.pessoas.filter((pessoa) =>
            pessoaIds.has(pessoa.id)
          )
        : [],
      lancamentos,
    };
...
export function criarBackup(
  ...
  for (const chave of obterChavesDoAplicativo()) {
    ...
    dados[chave] = prepararParaBackup(
      chave,
      valor,
      filtro
    );
...
export function baixarBackup(
  ...
  link.href = url;
  link.download =
    `financas-backup-${dataAtual}.json`;
...
export function restaurarBackup(backup) {
  ...
  for (
    const [chave, valor] of Object.entries(backup.dados)
  ) {
    ...
    for (const chave of area.chaves) {
      ...
      if (chave === "financas_transacoes") {
        localStorage.setItem(
          chave,
          JSON.stringify(
            filtrarLista(valor, filtro, false)
          )
        );
        quantidade += 1;
      }
...
export const restaurarBackupCompleto =
  restaurarBackup;